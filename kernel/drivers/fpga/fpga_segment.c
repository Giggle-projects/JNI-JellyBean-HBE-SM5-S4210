...
#define	SEGMENT_NAME		"fpga_segment"
...
int fpga_segment_open (struct inode *inode, struct file *filp)
{
...
int fpga_segment_release (struct inode *inode, struct file *filp)
{
...
ssize_t fpga_segment_write(struct file *inode, const char *gdata, size_t length, loff_t *off_what)
{
...
static int fpga_segment_ioctl(struct file *flip, unsigned int cmd, unsigned long arg)
{
    switch(cmd) {
	case MODE_0_TIMER_FORM:
	    mode_select=0x00;
...
struct file_operations fpga_segment_fops = 
{
	.owner		= THIS_MODULE,
...
static struct miscdevice fpga_seg_driver = {
	.fops	= &fpga_segment_fops,
	.name	= SEGMENT_NAME,
	.minor 	= MISC_DYNAMIC_MINOR,
};

int fpga_segment_init(void)
{
	misc_register(&fpga_seg_driver);
	printk("driver: %s DRIVER INIT\n", SEGMENT_NAME);
	return 0;
}

void fpga_segment_exit(void)
{
	misc_deregister(&fpga_seg_driver);

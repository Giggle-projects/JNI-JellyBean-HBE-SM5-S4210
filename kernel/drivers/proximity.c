...
static int proximity_device_ioctl( struct file*, unsigned int, unsigned long ); //update park for unlocked_ioctl
...
static struct file_operations proximity_device_fops =
{
        ...
        .unlocked_ioctl = proximity_device_ioctl,
...
// read proximity data
	distance = s3c_adc_read(client,ADC_CHANNEL);
...
static int proximity_device_ioctl( struct file* file, unsigned int cmd, unsigned long arg)
{
...
//added park for adc
	...
	if(IS_ERR(client))
	{
		printk(KERN_ERR "proximity.c failed to register adc \n");
	}
...
#else
	printk("HBE-SM7-S4412 PROXIMITY Device Driver initialized Ver!! 1.0 \n");
#endif

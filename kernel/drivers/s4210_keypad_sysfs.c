//
// 
//  S4210 Board : S4210 Keypad Interface driver 
// 
//
...
int	s4210_keypad_sysfs_create(struct platform_device *pdev);
void	s4210_keypad_sysfs_remove(struct platform_device *pdev);	
...
static struct attribute *s4210_keypad_sysfs_entries[] = {
...
static struct attribute_group s4210_keypad_attr_group = {
	.name   = NULL,
	.attrs  = s4210_keypad_sysfs_entries,
};
...
static 	ssize_t set_sampling_rate(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(val > 20)	s4210_keypad.sampling_rate = 2;
    else if(val > 10)	s4210_keypad.sampling_rate = 1;
    else		s4210_keypad.sampling_rate = 0;
...
static 	ssize_t set_poweroff_control(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(val > 3)	s4210_keypad.sampling_rate = 2;
    else if(val > 1)	s4210_keypad.sampling_rate = 1;
    else		s4210_keypad.sampling_rate = 0;
...
{
	// variable init
	s4210_keypad.sampling_rate	= 0;	// 10 msec sampling
	s4210_keypad.poweroff_time	= 1;	// 3 sec

	return sysfs_create_group(&pdev->dev.kobj, &s4210_keypad_attr_group);
}

void	s4210_keypad_sysfs_remove(struct platform_device *pdev)	
{
    sysfs_remove_group(&pdev->dev.kobj, &s4210_keypad_attr_group);
}
